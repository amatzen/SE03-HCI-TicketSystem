<template>
    ...
    <section class="w-full grid grid-cols-4 gap-12">
      ...
      >
        <h2 class="text-xl font-bold">
          <span
            class="inline-block w-3 h-3 bg-gray-700 rounded-full"
  ...
  <Observer>
    <TicketModal
      v-if="ticketStore.activeTicket !== null"
...
export default {
  ...
  setup(props) {
    ...
    const getTicketsByColumn = (col) =>
      ticketStore.value.ticketsList.filter(
        (x) => JSON.stringify(col) === JSON.stringify(x.status)
      );
